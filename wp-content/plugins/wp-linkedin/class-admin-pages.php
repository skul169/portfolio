...
class WPLinkedInAdmin {
	...
	function admin_notices() {
		if (!WP_LINKEDIN_APPKEY && (current_user_can('manage_options')) && !is_multisite()) {
			...
			<div class="notice notice-error"><p><?php echo $notice; ?></p></div><?php
			return;
		}
		...
		if (!$linkedin->is_access_token_valid()) {
			...
			<div class="notice notice-warning"><p><?php echo $notice; ?></p></div><?php
		}
		...
		if (isset($_GET['cache_cleared'])) { ?>
			<div class="notice notice-success"><p><?php _e('The cache has been cleared.', 'wp-linkedin'); ?></p></div><?php
		}
	}
