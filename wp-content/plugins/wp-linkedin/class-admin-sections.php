...
class WPLinkedInAPISection extends WPLinkedInAdminSection {
	...
	protected function register_settings() {
		...
		if (is_multisite()) {
			$this->add_options_field('redirect_through_network', __('Redirect through network', 'wp-linkedin'));
		}
	...
	function field_redirect_through_network() { ?>
		<label><input type="checkbox" name="wp-linkedin_redirect_through_network"
			value="1" <?php checked(LINKEDIN_REDIRECT_THROUGH_NETWORK); ?> />&nbsp;
			<?php _e('Uncheck this option if you need each network site to register its own redirect uri, for example if you want to hide that you are on a network.', 'wp-linkedin') ?>
			<?php _e('This will require that you register each site\'s redirect uri with the Linked API instead of just registering one for the network.', 'wp-linkedin') ?>
			<?php _e('A typical redirect uri will be http://mysite.com/oauth/linkedin', 'wp-linkedin') ?></label><?php
	}
...
class WPLinkedInExtensionsSection extends WPLinkedInAdminSection {
	...
	function section_header() { ?>
		...
		<div class="extension"><div class="extension-inner">
			<strong><a href="http://vdvn.me/p39h"><?php _e('WP LinkedIn Extensions Bundle', 'wp-linkedin'); ?></a></strong>
			<p><?php _e('All the WP LinkedIn extensions for a reduced price.', 'wp-linkedin'); ?></p>
			<span class="submit"><a class="button button-primary" href="<?php $this->download_link(4229); ?>"><?php _e('Get it!', 'wp-linkedin'); ?></a></span>
		</div></div>
		<div class="extension"><div class="extension-inner">
