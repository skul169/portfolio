...
class WPLinkedInConnection {
	...
	public function set_access_token($code, $redirect_uri=false) {
		$response = $this->retrieve_access_token($code, $redirect_uri);

		if (!is_wp_error($response)) {
			$this->set_cache('invalid_token_mail_sent', false);
			$this->access_token = $response->access_token;
			return $this->set_cache('oauthtoken', $response->access_token, $response->expires_in);
		} else {
			return $response;
		}
	}
	...
	public function retrieve_access_token($code, $redirect_uri=false) {
		...
		$response = wp_remote_get($url, array('sslverify' => LINKEDIN_SSL_VERIFYPEER, 'body' => $params));
		if (!is_wp_error($response)) {
	...
	public function get_authorization_url($redirect_uri=false) {
		...
		return 'https://www.linkedin.com/oauth/v2/authorization?' . http_build_query(array(
				'response_type' => 'code',
				'client_id' => $this->app_key,
				'scope' => implode(' ', $scope),
	...
	public function send_invalid_token_email() {
		if (LINKEDIN_SENDMAIL_ON_TOKEN_EXPIRY && !$this->get_cache('invalid_token_mail_sent')) {
			...
			$subject = "[$blog_name] " . __('Invalid or expired access token', 'wp-linkedin');
