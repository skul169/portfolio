<?php
/**
 * This code is a library used only by the WP LinkedIn's premium extensions in
 * order to determine if there are updates available for them. To do so the
 * updater will contact the server at vedovini.net using HTTPS.
 *
 * The WP LinkedIn plugin itself does not execute this code as it is exclusively
 * relying on the WordPress.org plugin repository for updates.
 */
...
class VDVNPluginUpdater {
	...
	function admin_notices() {
		...
		if (current_user_can($capability)) {
			...
			if (isset($error)) { ?>
				<div class="notice notice-warning"><p><?php _e($this->plugin_name, 'wp-linkedin'); ?>: <?php echo $error; ?></p></div><?php
			}
		}
	}
