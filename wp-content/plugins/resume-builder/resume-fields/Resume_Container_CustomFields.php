 ...
 *
 **/
class Resume_Container_CustomFields extends Resume_Container {
	/**
	...
	function save($post_id) {
		...
		do_action('resume_after_save_custom_fields', $post_id);
	}
	...
	function verify_unique_field_name($name) {
		if ( empty($this->settings['post_type']) ) {
			throw new Resume_Exception ('Panel instance is not setup correctly (missing post type)');
		}
	...
	function show_on_level($level) {
		if ($level < 0 ) {
			throw new Resume_Exception('Invalid level limitation (' . $level . ')');
		}
...
} // END Resume_Container_CustomFields 
