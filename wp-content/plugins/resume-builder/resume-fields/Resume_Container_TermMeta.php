...
class Resume_Container_TermMeta extends Resume_Container {
	/**
	...
	function __construct($title) {
		...
		if ( !$this->get_datastore() ) {
			$this->set_datastore(new Resume_DataStore_TermMeta());
		}
	}
	...
	function save($term_id) {
		...
		do_action('resume_after_save_term_meta', $term_id);
	}
	...
	function verify_unique_field_name($name) {
		if ( empty($this->settings['taxonomy']) ) {
			throw new Resume_Exception ('Panel instance is not setup correctly (missing taxonomy)');
		}
