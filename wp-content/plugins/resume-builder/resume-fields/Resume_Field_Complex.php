<?php 
class Resume_Field_Complex extends Resume_Field {
	...
	function init() {
		$this->labels = array(
			'singular_name'=>__('Entry', 'rbf'),
			'plural_name'=>__('Entries', 'rbf'),
		);
	...
	function add_fields() {
		...
		if ( array_key_exists('_' . $name, $this->groups) ) {
			throw new Resume_Exception('Group with name "' . $name . '" in Complex Field "' . $this->get_label() . '" already exists.');
		} else {
			$group = new Resume_Field_Group();
			$group->set_name( $name );
	...
	function set_value_from_input($input = null) {
		...
		foreach ($input_groups as $values) {
			...
			foreach ($group_fields as $field) {
				...
				if ( is_a($tmp_field, 'Resume_Field_Complex') ) {
	...
	function process_loaded_values($group_rows) {
		...
		foreach ($input_groups as $index => $values) {
			...
			foreach ($group_fields as $field) {
				...
				if ( is_a($field, 'Resume_Field_Complex') ) {
	...
	function template() {
		?>
		<div class="resume-subcontainer resume-grid {{ multiple_groups ? 'multiple-groups' : '' }}">
	
			<div class="resume-empty-row">
				{{{ rbfl10n.complex_no_rows.replace('%s', labels.plural_name) }}}
			</div>

			<div class="resume-groups-holder layout-{{ layout }}"></div>

			<div class="resume-actions">
				<div class="resume-button">
					<a href="#" class="button" data-group="{{{ multiple_groups ? '' : groups[0].name }}}">
						{{{ rbfl10n.complex_add_button.replace('%s', labels.singular_name) }}}
						{{{ multiple_groups ? '&#8681;' : '' }}}
					</a>
	...
	function template_group() {
		?>
		<div id="resume-{{{ complex_name }}}-complex-container" class="resume-row resume-group-row" data-group-id="{{ id }}">
			<input type="hidden" name="{{{ complex_name + '[' + index + ']' }}}[group]" value="{{ name }}" />

			<div class="resume-drag-handle">
				<span class="group-number">{{{ order + 1 }}}</span><span class="group-name">{{{ label }}}</span>
			</div>
			<div class="resume-group-actions">
				<a class="resume-btn-collapse" href="#" title="<?php esc_attr_e('Collapse/Expand', 'rbf'); ?>"><?php _e('Collapse/Expand', 'rbf'); ?></a>
				<a class="resume-btn-duplicate" href="#" title="<?php esc_attr_e('Clone', 'rbf'); ?>"><?php _e('Clone', 'rbf'); ?></a>
				<a class="resume-btn-remove" href="#" title="<?php esc_attr_e('Remove', 'rbf'); ?>"><?php _e('Remove', 'rbf'); ?></a>
			</div>

			<div class="fields-container">
				<# _.each(fields, function(field) { #>
					<div class="resume-row resume-subrow subrow-{{{ field.type }}} {{{ field.classes.join(' ') }}}">
						<label for="{{{ complex_id + '-' + field.id + '-' + index }}}">
							{{ field.label }}

							<# if (field.required) { #>
								 <span class="resume-required">*</span>
							<# } #>
						</label>
	...
	function set_layout($layout) {
		_doing_it_wrong(__METHOD__, __('Complex field layouts are deprecated, please use <code>set_width()</code> instead.', 'rbf'), null);

		if ( !in_array($layout, array(self::LAYOUT_TABLE, self::LAYOUT_LIST)) ) {
			throw new Resume_Exception('Incorrect layout specifier. Available values are "<code>' . self::LAYOUT_TABLE . '</code>" and "<code>' . self::LAYOUT_LIST . '</code>"');
		}
...
class Resume_Field_Group {
	...
	function add_fields($fields) {
		foreach ($fields as $field) {
			if ( !is_a($field, 'Resume_Field') ) {
				throw new Resume_Exception('Object must be of type Resume_Field');
			}

			// verify name validity
			if ( preg_match('~_\d+~', $field->get_name()) ) {
				throw new Resume_Exception ('Subfield names cannot contain underscore followed by a digit(s). Replace "' . ltrim($field->get_name(), '_') . '" with "' . ltrim(preg_replace('~_+(\d+)~', '$1', $field->get_name()), '_') . '"');
			}
	...
	function verify_unique_field_name($name) {
		if ( in_array($name, $this->registered_field_names) ) {
			throw new Resume_Exception ('Field name "' . $name . '" already registered');
		}
