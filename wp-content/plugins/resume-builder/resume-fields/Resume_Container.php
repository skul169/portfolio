...
add_action('admin_print_scripts', array('Resume_Container', 'admin_hook_scripts'));
add_action('admin_print_styles', array('Resume_Container', 'admin_hook_styles'));
 ...
 *
 **/
abstract class Resume_Container {
	/**
	...
	static function factory($type, $name) {
		...
		$class = 'Resume_Container_' . $type;

		if (!class_exists($class)) {
			throw new Resume_Exception ('Unknown container "' . $type . '".');
		}
	...
	static function add_active_container($container) {
		...
		do_action('rbf_container_activated', $container);
	}
	...
	static function add_active_field($field) {
		...
		do_action('rbf_field_activated', $field);
	}
	...
	function setup($settings = array()) {
		if ( $this->setup_ready ) {
			throw new Resume_Exception ('Panel "' . $this->title . '" already setup');
		}
	...
	function check_setup_settings(&$settings = array()) {
		...
		if ( !empty($invalid_settings) ) {
			throw new Resume_Exception ('Invalid settings supplied to setup(): "' . implode('", "', array_keys($invalid_settings)) . '"');
		}
	}
	 ...
	 * must be instances of Resume_Field and their names should be unique for all
	 * Resume containers.
	...
	function verify_unique_panel_id($id) {
		if ( in_array($id, self::$registered_panel_ids) ) {
			throw new Resume_Exception ('Panel ID "' . $id .'" already registered');
		}
	...
	function get_nonce_name() {
		return 'resume_panel_' . $this->id . '_nonce';
	}
	...
	function template_tabs() {
		...
		<div class="resume-tabs">
			<ul class="resume-tabs-nav">
				<# _.each(tabs, function (tab, tabName) { #>
					<li><a href="#">{{{ tabName }}}</a></li>
				<# }); #>
			</ul> 

			<div class="resume-tabs-body">
				<# _.each(tabs, function (tab) { #>
					<div class="resume-fields-collection resume-tab">
						{{{ tab.html }}}
					</div>
				<# }); #>
			</div>
	...
	static function admin_hook_scripts() {
		wp_enqueue_script('resume-containers', RESUME_PLUGIN_URL . '/js/containers.js', array('resume-app'));

		wp_localize_script('resume-containers', 'resume_containers_l10n',
			array(
				'please_fill_the_required_fields' => __('Please fill out all required fields highlighted below.', 'rbf'),
				'changes_made_save_alert' => __('The changes you made will be lost if you navigate away from this page.', 'rbf'),
			)
		);
	}

	static function admin_hook_styles() {
		wp_enqueue_style('resume-main', RESUME_PLUGIN_URL . '/css/main.css');
	}

} // END Resume_Container 
