...
class Resume_Container_ThemeOptions extends Resume_Container {
	...
	function save($user_data = null) {
		...
		do_action('resume_after_save_theme_options', $user_data);
	...
	function init() {
		if ( !$this->settings['parent'] || $this->settings['parent'] == 'self' ) {
			...
			$this->settings['parent'] = 'rbfn-' . $clear_title . '.php';
		}

		if ( !$this->settings['file'] ) {
			$clear_title = $this->clear_string($this->title);
			$this->settings['file'] .= 'rbfn-' . $clear_title . '.php';
		}
	...
	function is_valid_save() {
		if ( !isset($_SERVER['REQUEST_METHOD']) || $_SERVER['REQUEST_METHOD'] != 'POST' ) {
			return false;
		} else if ( !wp_verify_nonce( rbf_request_param($this->get_nonce_name()), $this->get_nonce_name() ) ) {
			return false;
		}
	...
	function render() {
		if ( isset($_GET['settings-updated']) && $_GET['settings-updated'] == 'true' ) {
			$this->notifications[] = __('Settings saved.', 'rbf');
		}
	...
	function verify_unique_page() {
		...
		if ( !$parent ) {
			if ( isset(self::$registered_pages[$file]) ) {
				throw new Resume_Exception ('Page "' . $file . '" already registered');
			}
		...
		if ( !isset(self::$registered_pages[$parent]) ) {
			...
			throw new Resume_Exception ('Page "' . $file . '" with parent "' . $parent . '" is already registered. Please set a different file name using setup()');
		} else {
	...
	function set_page_parent($parent) {
		if ( is_a($parent, 'Resume_Container') ) {
			$parent = $parent->title;
		}
