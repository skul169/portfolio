...
abstract class Resume_Widget extends WP_Widget implements Resume_DataStore {
	...
	function setup($title, $description, $fields, $classname = '') {
		// require title
		if ( !$title ) {
			throw new Resume_Exception('Enter widget title');
		}
		...
		# Generate Widget ID
		$widget_ID = 'resume_' . preg_replace('~\s+~', '_', strtolower(trim(preg_replace('/[^a-zA-Z0-9]+/u', '', remove_accents($title)))));
	...
	function form($instance) {
		...
		Resume_Container::factory('widget', $this->id)
	...
	function verify_unique_widget_id($id) {
		if ( in_array($id, self::$registered_widget_ids) ) {
			throw new Resume_Exception ('Widget with ID "' . $id .'" already registered. Please change the widget title');
		}
