...
(function($) {
	...
	|
	*/
	resume.fields.Model = Backbone.Model.extend({
		defaults: {
			'error': false,
			'visible': true,
		...
		initialize: function() {
			var classes = ['resume-field', 'resume-' + this.get('type')]
			var width = this.get('width');

			if (this.get('lazyload')) {
				classes.push('resume-lazyload');
			}
		...
		validate: function(attrs, options) {
			if (this.isRequired() && !attrs.value) {
				return rbfl10n.message_required_field;
			}
		}
	});
	...
	|
	*/
	resume.fields.View = Backbone.View.extend({
		/*
		...
		conditionalLogicInit: function() {
			...
			var conditionalLogic = new resume.fields.ConditionalLogic({
		...
		toggleVisibility: function() {
			...
			var $holder = this.$el.closest('.resume-field');
		...
		toggleError: function() {
			...
			var $holder = this.$el.closest('.resume-field');
			var $errorHolder = $holder.find('> .resume-error');
			var hasError = !!errorText;

			$holder.toggleClass('resume-highlight', hasError);
		...
		setWidth: function() {
			...
			if (width && _.isNumber(width)) {
				this.$el.closest('.resume-field').css('width', width + '%');
			}
		},
		...
		updateClass: function(model) {
			...
			this.$el.closest('.resume-field').addClass(classes.join(' '));
		},
	...
	| 
	*/
	resume.fields.Collection = Backbone.Collection.extend({
		model: function(attrs, options) {
			var FieldModel = resume.fields.Model[attrs.type];

			// Set the field model. If the model is not found, fallback to the base model
			if ( _.isUndefined(FieldModel) ) {
				FieldModel = resume.fields.Model; // Fallback to the base model
			}
	...
	|
	*/
	resume.fields.ConditionalLogic = Backbone.Model.extend({
		defaults: {
	...
	// Map VIEW
	resume.fields.View.Map = resume.fields.View.extend({
		events: {
		...
		initialize: function() {
			resume.fields.View.prototype.initialize.apply(this);
		...
		initMap: function() {
			...
			var $element = this.$el;
			var $mapContainer = $element.find('.resume-map-canvas');
		...
		geocodeAddress: function(model) {
			...
			geocoder.geocode( { 'address': address }, function(results, status) {
				if (status == google.maps.GeocoderStatus.OK) {
					...
					alert(rbfl10n.geocode_zero_results);
				} else {
					alert(rbfl10n.geocode_not_successful + status);
				}
			});
		},
	...
	// RichText VIEW
	resume.fields.View.RichText = resume.fields.View.extend({
		initialize: function() {
			resume.fields.View.prototype.initialize.apply(this);
		...
		get_mceInit: function(){
			var $field = this.$el;
			var id = this.model.get('id');
			var mceInit = $.extend({}, tinyMCEPreInit.mceInit.resume_settings);
			var isTouchscreen = resume.views.main.$body.hasClass('touchscreen');
		...
		get_qtInit: function(){
			var qtInit = $.extend({}, tinyMCEPreInit.qtInit.resume_settings);
		...
		enableEditor: function() {
			...
			var $editor = this.$('.resume-wysiwyg');
	...
	// Date VIEW
	resume.fields.View.Date = resume.fields.View.extend({
		initialize: function() {
			resume.fields.View.prototype.initialize.apply(this);
		...
		initDatePicker: function() {
			var $field = this.$('.resume-datepicker');
			var $trigger = this.$('.resume-datepicker-trigger');
			...
			var args = {
				...
				beforeShow: function(input, inst) {
					$('#ui-datepicker-div').addClass('resume-jquery-ui');
				}
			};
	...
	// Color VIEW
	resume.fields.View.Color = resume.fields.View.extend({
		events: _.extend({}, resume.fields.View.prototype.events, {
			'click .pickcolor.button': 'focusField',
			'focus input.resume-color': 'showColorPicker',
			'blur input.resume-color': 'hideColorPicker'
		}),
		...
		initColorPicker: function() {
			var _this = this;
			var $field = this.$field = this.$('input.resume-color');
	...
	// Select MODEL
	resume.fields.Model.Select = resume.fields.Model.extend({
		initialize: function() {
			resume.fields.Model.prototype.initialize.apply(this);
		...
		validate: function(attrs, options) {
			...
			if (this.isRequired() && (!value || value === '0')) {
				return rbfl10n.message_choose_option;
			}
		}
	});
	...
	// Gravity Form MODEL
	resume.fields.Model.GravityForm = resume.fields.Model.Select.extend();
	...
	// Sidebar MODEL
	resume.fields.Model.Sidebar = resume.fields.Model.Select.extend({
		initialize: function() {
			this.setSidebarOptions();

			resume.fields.Model.Select.prototype.initialize.apply(this);
		},
		...
		setSidebarOptions: function() {
			...
			resume.collections.sidebars.each(function(model) {
		...
		validate: function(attrs, options) {
			return resume.fields.Model.prototype.validate.apply(this, arguments);
		}
	});

	// Sidebar VIEW
	resume.fields.View.Sidebar = resume.fields.View.extend({
		events: _.extend({}, resume.fields.View.prototype.events, {
		...
		initialize: function() {
			...
			this.listenTo(resume.collections.sidebars, 'add', this.addSidebarOption);
		},
		...
		addNewSidebar: function(event) {
			...
			if (sidebarName) {
				var sidebarExists = resume.collections.sidebars.findWhere({
					name: sidebarName
				});

				if (!sidebarExists) {
					// Add the new sidebar to the sidebars collection
					resume.collections.sidebars.add({
						name: sidebarName
					});
				}
	...
	resume.fields.Model.ChooseSidebar = resume.fields.Model.Sidebar.extend({
		setSidebarOptions: function() {}
	});

	// Choose Sidebar VIEW
	resume.fields.View.ChooseSidebar = resume.fields.View.Sidebar.extend({
		initialize: function() {
			resume.fields.View.Sidebar.prototype.initialize.apply(this);

			// Remove the addSidebarOption listener, we are not using the sidebars collection for this field.
			this.stopListening(resume.collections.sidebars, 'add', this.addSidebarOption);
		},
	...
	// File VIEW
	resume.fields.View.File = resume.fields.View.extend({
		events: _.extend({}, resume.fields.View.prototype.events, {
		...
		loadDescriptionTemplate: function() {
			var type = this.model.get('type');
			var descTemplate = resume.template(type + '-Description');
		...
		openMedia: function(event) {
			...
			mediaTypes[type] = wp.media.frames.rbfMediaField = wp.media({
				title: windowLabel ? windowLabel : rbfl10n.title,
		...
		updateInput: function(model) {
			var $input = this.$('input.resume-file-field');
		...
		updateView: function(model) {
			...
			this.$('.resume-view_file').attr('href', url);
			this.$('.resume-description').toggleClass('hidden', !url);
		}
	});
	...
	// Attachment VIEW
	resume.fields.View.Attachment = resume.fields.View.File.extend({
		events: _.extend({}, resume.fields.View.File.prototype.events, {
			'click .resume-file-remove': 'removeFile'
		}),

		initialize: function() {
			resume.fields.View.File.prototype.initialize.apply(this);
		...
		updateThumb: function(model) {
			...
			this.$('.resume-attachment-preview').toggleClass('hidden', !thumbUrl);
		},

		removeFile: function(event) {
			this.$('.resume-description').addClass('hidden');
			this.$('.resume-attachment-preview').addClass('hidden');
			this.$('input.resume-file-field').attr('value', '').trigger('change');
	...
	// Image VIEW
	resume.fields.View.Image = resume.fields.View.Attachment.extend();
	...
	// Checkbox VIEW
	resume.fields.View.Checkbox = resume.fields.View.extend({
		sync: function(event) {
	...
	// Set MODEL
	resume.fields.Model.Set = resume.fields.Model.extend({
		validate: function(attrs, options) {
	...
	// Set VIEW
	resume.fields.View.Set = resume.fields.View.extend({
		initialize: function() {
			resume.fields.View.prototype.initialize.apply(this);
		...
		showAll: function() {
			this.$('a.resume-set-showall').one('click', function (event) {
	...
	// Relationship MODEL
	resume.fields.Model.Relationship = resume.fields.Model.extend({
		validate: function(attrs, options) {
			if (this.isRequired() && _.isEmpty(attrs.value)) {
				return rbfl10n.message_required_field;
			}
	...
	// Relationship VIEW
	resume.fields.View.Relationship = resume.fields.View.extend({
		...
		addItem: function(event) {
			...
			var liTemplate = resume.template(this.model.get('type') + '_item');
			var newLi;
			...
			if ( max > 0 && this.selectedItems.length >= max ) {
				alert(rbfl10n.max_num_items_reached.replace('%s', max));
				return false;
			};
	...
	// Association MODEL
	resume.fields.Model.Association = resume.fields.Model.Relationship.extend({
		initialize: function() {
			resume.fields.Model.Relationship.prototype.initialize.apply(this);

			this.addClass('resume-Relationship');
		}
	});

	// Association VIEW
	resume.fields.View.Association = resume.fields.View.Relationship.extend({
		buildItemValue: function(id, type, subtype) {
			var sep = ':';
	...
	// Complex MODEL
	resume.fields.Model.Complex = resume.fields.Model.extend({
		defaults: _.extend({}, resume.fields.Model.prototype.defaults, {
			'index': 0,
			'force_required': true
		}),
		...
		validate: function(attrs, options) {
			...
			if ( !groupsValid ) {
				return rbfl10n.message_form_validation_failed;
			}
			...
			if ( !minValid ) {
				...
				return rbfl10n.complex_min_num_rows_not_reached
					.replace( '%d', min )
	...
	// Complex VIEW
	resume.fields.View.Complex = resume.fields.View.extend({
		events: {
			'click > .resume-subcontainer > .resume-actions a': 'buttonAction',
			'click > .resume-subcontainer > .resume-empty-row a': 'buttonAction'
		},
		...
		initialize: function() {
			...
			this.groupsCollection = new resume.fields.Collection.Group;
			...
			this.on('propagate', function(event) {
				resume.containers.View.prototype.eventPropagator.apply(this, [this.groupsCollection, event]);
			});
		...
		setDOMVariables: function() {
			this.$actions = this.$('.resume-actions');
			this.$introRow = this.$('.resume-empty-row');
			this.$groupsList = this.$actions.find('ul');
			this.$groupsHolder = this.$('.resume-groups-holder');
		},
		...
		sortable: function() {
			...
			this.$groupsHolder.sortable({
				items : '> .resume-group-row',
				handle: '.resume-drag-handle',
				placeholder: 'resume-group-row ui-placeholder-highlight',
				start: function(event, ui) {
					_this.$groupsHolder.addClass('resume-container-shrank');

					ui.item.groupID = ui.item.data('group-id');
					ui.item.groupView = resume.views[ui.item.groupID];
				...
				stop: function(event, ui) {
					_this.$groupsHolder.removeClass('resume-container-shrank');
		...
		hideGroupsListListener: function() {
			...
			resume.views.main.$body.on('click', function(event) {
		...
		renderGroup: function(model) {
			...
			resume.views[id] = new resume.fields.View.Complex.Group({
				el: this.$groupsHolder,
				model: model
			});

			resume.views[id].on('layoutUpdated', function() {
				_this.trigger('layoutUpdated');
			});

			resume.views[id].render(this.model);
	...
	// Group MODEL
	resume.fields.Model.Complex.Group = Backbone.Model.extend({
		defaults: {
			'order': null,
			'index': null,
			'collapsed': false
		...
		validate: function(attrs, options) {
			return resume.containers.Model.prototype.validate.apply(this, arguments);
		}
	});

	// Group VIEW
	resume.fields.View.Complex.Group = Backbone.View.extend({
		events: {
			'click .resume-group-actions:first a.resume-btn-remove': 'removeGroup',
			'click .resume-group-actions:first a.resume-btn-collapse': 'collapseGroup',
			'click .resume-group-actions:first a.resume-btn-duplicate': 'duplicateGroup'
		},
		...
		initialize: function() {
			...
			this.on('propagate', function(event) {
				resume.containers.View.prototype.eventPropagator.apply(this, [this.fieldsCollection, event]);
			});
		...
		updateOrderNumber: function(model) {
			...
			this.$('> .resume-drag-handle .group-number').text(groupOrder + 1);
		},
		...
		render: function(complexModel) {
			...
			var template = resume.template('Complex-Group');
			...
			// At this moment this.$el points to the groups holder ( .resume-groups-holder )
			var $holder = this.$el;
		...
		renderField: function(model) {
			resume.containers.View.prototype.renderField.apply(this, arguments);
	...
	resume.fields.Collection.Group = Backbone.Collection.extend({
		model: resume.fields.Model.Complex.Group,
