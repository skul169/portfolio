...
class BestEditor_Options_Page {
	...
	function __construct() {
		...
		$this->plugin_name = __('Best Editor', $this->plugin_slug);
		$this->page_title  = sprintf(__('%s Settings', $this->plugin_slug), $this->plugin_name);
	...
	function add_admin_notices() {
		...
		<div id="notice" class="update-nag">
			<?php echo sprintf(__("%s is not configured, ", $this->plugin_slug), $this->plugin_name); ?>
			<a href="<?php menu_page_url($this->plugin_slug, true); ?>"><?php
				_e('Please configure it now!', $this->plugin_slug); ?></a>
		</div>
		<?php
	}
	...
	function add_setting_page_help() {
		...
		$help_content = '<p>' .
		                sprintf(__('Use this page to configure %s plugin', $this->plugin_slug), $this->plugin_name) .
		                '</p>' .
		                '';
		...
		$screen->set_help_sidebar(
			'<p><strong>' .
			__('For more information:', $this->plugin_slug) .
			'</strong></p>' .
			'<p><a href="https://esitedesign.com" target="_blank">' .
			__('Visit Our Website!', $this->plugin_slug) .
			'</a></p>'
		);
	}
	...
	function register_settings() { // whitelist options
		...
		add_settings_section(
			...
			__('Settings for loading libraries', $this->plugin_slug), // title heading for the section
			function ( $args ) { ?>
				<p id="<?php echo esc_attr($args['id']); ?>"><?php _e(
		...
		add_settings_field(
			...
			__('Load Font Awesome library in your site frontend', $this->plugin_slug), // Title as lable for field
			function ( $args ) {
				$best_editor       = get_option($this->plugin_slug);
				$check_fontawesome = isset( $best_editor['fontawesome'] ) ? $best_editor['fontawesome'] : false;
				?>
				<input type="checkbox" name="besteditor[fontawesome]" id="fontawesome" value="true" <?php checked(
					true,
					$check_fontawesome
				); ?> />
				<?php
		...
		add_settings_field(
			...
			__('Load Bootstrap 3.3.7 in your site frontend', $this->plugin_slug), // Title as lable for field
			function ( $args ) {
				$best_editor     = get_option('besteditor');
				$check_bootstrap = isset( $best_editor['bootstrap'] ) ? $best_editor['bootstrap'] : false;
				?>
				<input type="checkbox" name="besteditor[bootstrap]" id="bootstrap" value="true" <?php checked(
					true,
					$check_bootstrap
				); ?> />
				<?php
	...
	public function process_inputs( $input ) {
		...
		if ( false ) {
			add_settings_error(
				...
				__('Entered options are not valid!', $this->plugin_slug),
	...
	function settings_page_content() {
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				<p><?php _e(
						'Best Editor automatically loads FontAwesome and Bootstrap 3.3.7 in editor area, But if you like to use Font Awesome and Bootstrap elements,<br />
								 You should load these libraries in your site frontend, you can enable them in following (if your theme doesn\'t load these libraries)',
						$this->plugin_slug
					); ?></p>
				<?php
				submit_button();
